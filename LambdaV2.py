...
def FindAlphabet(string):
  ...
  result2 = [chr(955)]
  #Dit is de eerste letter van het alphabet, meestal de lambda
...
def Replace(target,goal,data):
  #Deze functie zoekt een waarde 'target' in data en zet deze om in 'doel'  
...
def FindNumber(target,data):
    #Deze functie telt het aantal verschijningen van 'target' in 'data'
  number = 0 
  for i in data:
...
def DeepCopy(data,target):
  #Maakt een kopie van een lijst, ook indien het een lijst van lijsten is.
  for i in range(len(data)):
    if isinstance(data[i],list):
      target.append([])
...
def FindReduce(data,route):
    #Dit checkt of een lambdaterm Beta-reducibel is
    #en geeft de manier waarop, terug in de variabele route. 
...
class LambdaTerm:
    ...
    def __str__(self):
        #Zet onze Bruijn index weer om in een string(een lambda term
        #zoals we hem kennen)
    ...
    def __call__(self, target, goal):
        #Deze functie is voor het veranderen van vaste variabele namen.
        #(De alpha-conversie)
    ...
    def resolveConflict(self):
        # Deze functie lost conflicten in gelijke letters op.
      CurrentAlphabet = []
    ...
    def reduceStep(self):
        #Deze functie behandelt de Beta-reductie.
        route = []
        first = []
        second = []
        upper = []
        ...
        #Indien de lambdaterm Beta-reducibel is.
        if FindReduce(self.BruijnIndex,route):
        #Maak een kopie van de Bruijn index.
        ...
        #Zoek een lambda in de Bruijn index
